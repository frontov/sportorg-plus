...
class FilterDialog(QDialog):
    def __init__(self, table=None):
        ...
        for i in range(len(headers)):
            ...
            self.combo_action = AdvComboBox(
                self, [_('equal to'), _('contain')], max_width=110
            )
            self.combo_action.setEditable(False)
        ...
        self.layout.addWidget(self.max_rows_count_label, len(headers), 0, 1, 2)
        self.layout.addWidget(self.max_rows_count_spin_box, len(headers), 2)
        ...
        self.button_clear = QPushButton(text=_('Clear'))
        ...
        try:
            ...
            if self.table:
                ...
                for i in range(len(headers)):
                    ...
                    if len(value):
                        proxy_model.set_filter_for_column(i, value, action == _('equal to'))
                        if headers[i] == _('Group'):
                            PersonEditDialog.GROUP_NAME = value
                        elif headers[i] == _('Team'):
                            PersonEditDialog.TEAM_NAME = value
    ...
    def recover_filter(self):
        ...
        for filter_key in self.table.model().filter.keys():
            ...
            strict_check = self.table.model().filter.get(filter_key)[1]
            action = _('equal to') if strict_check else _('contain')
